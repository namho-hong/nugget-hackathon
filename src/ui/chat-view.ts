...
import { getRoomDisplayName, resolveRoomOrThrow } from "../matrix/index.js";
...
export async function openChatView(
  ...
  await runInteractiveChat(client, room);
}
...
async function runInteractiveChat(client: MatrixClient, room: Room): Promise<void> {
  ...
  await new Promise<void>((resolve) => {
    ...
    const render = (): void => {
      ...
      const entries = getHistoryEntries(room);
      const historyLines = renderHistory(entries, width);
      ...
      const header = renderHeader(room, notice, width);
      const visibleInput = sliceDisplayTextFromEnd(inputBuffer, Math.max(1, width - 3));
      const composer = renderComposer(visibleInput, submitting, width);
    ...
    const submitText = async (rawText: string): Promise<void> => {
      ...
      if (text === "/help") {
        inputBuffer = "";
        setNotice("Commands: /help, /invite @user:server, /quit, /exit");
        return;
      }
...
function renderHeader(room: Room, notice: string, width: number): string[] {
  const title = `# ${sanitizeForTerminal(getRoomDisplayName(room))}`;
...
function renderHistory(entries: readonly HistoryEntry[], width: number): string[] {
  const lines: string[] = [];
  ...
  for (const entry of entries) {
    ...
    lines.push(...renderMessage(entry, width, grouped));
...
function renderMessage(message: RenderedMessage, width: number, grouped: boolean): string[] {
...
function renderSlashHelp(inputBuffer: string, width: number): string[] {
  ...
  return [
    fitDisplayText(
      `${DIM}/help  /invite @user:server  /quit  /exit${ANSI_RESET}`,
      width,
    ),
  ];
}

function renderComposer(input: string, submitting: boolean, width: number): string[] {
  const prompt = submitting ? "> sending..." : `> ${input}`;
  return [
    "",
...
function getHistoryEntries(room: Room): HistoryEntry[] {
  ...
  for (const event of room.getLiveTimeline().getEvents()) {
    ...
    const entry = buildHistoryEntry(room, event, eventId);
...
function padLines(lines: readonly string[], count: number, width: number): string[] {
  const padded = lines.map((line) => fitDisplayText(line, width));
